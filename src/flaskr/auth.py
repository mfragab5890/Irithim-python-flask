...
def get_permissions(id):
    ...
    if role:
        ...
        user_assigned_lists = UserLists.query.with_entities(UserLists.list_id).filter(UserLists.user_id == id).all()

        # get all user creatd lists
        user_owned_lists = List.query.with_entities(List.id).filter(List.creator_id == id).all()

        # get all cards on user lists or cards he created
        all_user_cards = db.session.query(User) \
            .join(Cards) \
            .filter(Cards.list_id.in_(user_owned_lists) | Cards.creator_id == id) \
        ...
        all_user_comments = db.session.query(User) \
            .join(Comments) \
            .filter(Comments.card_id.in_(all_user_cards) | Comments.creator_id == id) \
        ...
        all_user_replies = db.session.query(User) \
            .join(Replies) \
            .filter(Replies.comment_id.in_(all_user_comments) | Replies.creator_id == id) \
        ...
        payload = {
            ...
            'permissions': {
                ...
                'unassign_member_list': user_owned_lists,
                'create_card': 'All',
    ...
    # if role is False the user is a member
        ...
        all_user_created_cards = db.session.query(User) \
            .join(Cards) \
            .filter(Cards.list_id.in_(Cards.creator_id == id)) \
            .with_entities(Cards.id) \
            .all()
